...
//		CurrentUserSession currSess = currSession.findByPrivateKey(key);
//		if (currSess != null && currSess.getRole().equalsIgnoreCase("admin")) {
			System.out.println("In the service class id is "+restaurantId);
			Optional<Restaurant> opt = resRepo.findById(restaurantId);
			if (opt.isPresent()) {
				Restaurant newRes = opt.get();
				newRes.setRestaurantName(restaurant.getRestaurantName());
				newRes.setAddress(restaurant.getAddress());
				newRes.setContactNunber(restaurant.getContactNunber());
				newRes.setItemList(restaurant.getItemList());
				newRes.setManagerName(restaurant.getManagerName());
...
//			throw new LoginException("Login required");
	...
	@Override
	public List<Item> getRestaurantItemList(Integer restaurantId) throws RestaurantException {
		// TODO Auto-generated method stub
