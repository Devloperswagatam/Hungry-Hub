...
//			throw new LoginException("Admin login required");
	...
	@Override
	public Item updateItem(String key,Integer itemId ,ItemDTO itemDTO) throws ItemException, CategoryException, LoginException {
...
//			throw new LoginException("Admin login required");
	...
	@Override
	public List<Item> getItemsByCartId(Integer cartId) throws FoodCartException,ItemException,LoginException {
	    Optional<FoodCart> foodCartOpt = foodcartRepo.findById(cartId);
	    if (foodCartOpt.isEmpty()) {
	        throw new FoodCartException("Food Cart not found!");
	    }

	    FoodCart foodCart = foodCartOpt.get();
	    return foodCart.getItemList();
	}
